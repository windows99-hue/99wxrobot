...
#创建教程完毕的字典，防止重复教程
# help_finished = {}
help_finished = {"苏尤":True,"Windows":True}
close_robot = []
#初始提示
help_text = ''' 
...
直接跟我说话就可以和我聊天啦/n
祝你天天开心~[呲牙]/n
此版本为beta版本, 已接入张99本地运行的deepseek-v2/n
...
def get_reply(keyword, user_name):
    ...
    response = ollama.chat( #真正发送信息
        model='deepseek-r1:8b',
        messages=user_ai_content[user_name])
    response = response.message.content #获取回复
    response = response.split("</think>")[1] #无回车格式化

    user_ai_content[user_name].append({
            'role': 'assistant',
...
while not exit_status:
    ...
    if ated or we.Name:
        ...
        #留言系统
            ...
            reply = get_reply(last_msg, user_name)
        '''
        if not reply:
            print("没有需要回复的内容，跳过")
            continue
